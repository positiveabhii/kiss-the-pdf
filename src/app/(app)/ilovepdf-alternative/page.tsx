...
export const metadata: Metadata = {
  title: `Best Free ${siteConfig.name} Alternative to iLovePDF (No Upload) | ${siteConfig.name}`,
  description: `Looking for a free alternative to iLovePDF? ${siteConfig.name} processes PDFs locally in your browser with no file size limits and no account required.`,
  alternates: {
    canonical: `${siteConfig.url}/ilovepdf-alternative`,
  }
};
...
export default function CompetitorAlternativePage() {
  const jsonLdArticle = {
    ...
    "@type": "Article",
    "headline": `Best ${siteConfig.name} Alternative to iLovePDF`,
    "description": `Compare ${siteConfig.name} vs iLovePDF. Find out why our free, open-source PDF tools are the best alternative.`,
  };
    ...
    <div className="flex flex-col max-w-5xl mx-auto w-full space-y-12 py-8 px-4">
      ...
      <div className="space-y-4 pb-4 border-b border-slate-200/60 text-center">
        <h1 className="text-3xl sm:text-5xl font-bold text-slate-900 tracking-tight">
          The Best Free Alternative to <span className="text-blue-600">iLovePDF</span>
        </h1>
      ...
      <section className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        <div className="grid grid-cols-3 bg-slate-50 border-b border-slate-200 p-4 font-bold text-slate-800">
          <div>Feature</div>
          <div className="text-center">iLovePDF</div>
      ...
      <section className="space-y-6">
        <h2 className="text-2xl font-bold text-slate-900">Why choose {siteConfig.name} over iLovePDF?</h2>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="p-6 bg-blue-50 rounded-lg border border-blue-100">
            <h3 className="font-bold text-blue-900 mb-2">1. Your Files Never Leave Your Device</h3>
            <p className="text-blue-800 text-sm">Unlike iLovePDF, which requires you to upload documents to their servers, {siteConfig.name} uses advanced WebAssembly to process PDFs directly in your web browser. This means maximum privacy for sensitive data.</p>
          </div>
